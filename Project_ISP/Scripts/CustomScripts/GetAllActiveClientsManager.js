var GetAllActiveClientsManager = {
    Validation: function () {
    ...
    PrintAllActiveClientList: function () {
        
        var url = "/Excel/CreateReportForActiveClient";
        ...
        data = GetAllActiveClientsManager.addRequestVerificationToken(data);
        AppUtil.MakeAjaxCallsForAntiForgery(url, "POST", data, GetAllActiveClientsManager.PrintAllActiveClientListSuccess, GetAllActiveClientsManager.PrintAllActiveClientListFail);
    },
    PrintAllActiveClientListSuccess: function (data) {
    ...
    PrintAllActiveClientListFail: function (data) {
    ...
    InsertClientDetails: function () {
        ...
        var BillPaymentDate = AppUtil.GetIdValue("BillPaymentDate")//$('#BillPaymentDate').datepicker('getDate');
        ...
        AppUtil.MakeAjaxCall(url, "POST", datas, GetAllActiveClientsManager.InsertClientDetailsSuccess, GetAllActiveClientsManager.InsertClientDetailsFail);
    },
    ...
    UpdateClientDetails: function () {
        ...
        //var datas = JSON.stringify({ ClientClientDetails: ClientDetails, ClientTransaction: Transaction, ClientClientLineStatus: ClientLineStatus });
            AppUtil.MakeAjaxCall(url, "POST", datas, GetAllActiveClientsManager.UpdateClientDetailsSuccess, GetAllActiveClientsManager.UpdateClientDetailsFail);
        //}, 500);
    },
    UpdateClientDetailsSuccess: function (data) {
            ...
            if (data.ClientDetails) {
                ...
                $("#tblUsers>tbody>tr").each(function () {
                    ...
                    if (employeeID == parseEmployee[0].ClientDetailsID) {
                        ...
                        if (data.chkStatusFromRunningMonth === true && data.ClientLineStatus == "5") {
        ...
        GetAllActiveClientsManager.ClearClientDetailsModalInformation();
    ...
    GetClientDetailsByID: function (id) {
      ...
      //  setTimeout(function () {
            ...
            AppUtil.MakeAjaxCallsForAntiForgery(url, "POST", Data, GetAllActiveClientsManager.GetClientDetailsByIDSuccess, GetAllActiveClientsManager.GetClientDetailsByIDError);
     //   }, 500);
    ...
    GetClientDetailsByIDSuccess: function (data) {
        ...
        //$("#BillPaymentDate").val(AppUtil.ParseDate(Transaction[0].PaymentDate));
        $("#ClientSurvey").val(ClientDetails.ClientSurvey);
        $("#ConnectionDate").val(AppUtil.ParseDate(ClientDetails.ConnectionDate));
    ...
    SearchClientListByZone: function (SearchID, searchType) {
        ...
        AppUtil.MakeAjaxCallsForAntiForgery(url, "POST", data, GetAllActiveClientsManager.SearchClientListByZoneSuccess, GetAllActiveClientsManager.SearchClientListByZoneError);
    },
   ...
   //     $('#tblUsers').dataTable().fnDestroy();
    ...
    SearchActiveClientListByZone: function (SearchID, searchType) {
        
        if (SearchID === "") {
            SearchID = 0;
        }
        //AppUtil.ShowWaitingDialog();
        var url = "/Client/SearchActiveClientListByZone/";
        var AntiForgeryToken = $("input[name='__RequestVerificationToken']").val();
        var data = ({ SearchID: SearchID, searchType: searchType, __RequestVerificationToken: AntiForgeryToken });
        AppUtil.MakeAjaxCallsForAntiForgery(url, "POST", data, GetAllActiveClientsManager.SearchActiveClientListByZoneSuccess, GetAllActiveClientsManager.SearchActiveClientListByZoneError);
    },
    SearchActiveClientListByZoneSuccess: function (data) {
        ...
        $.each(listOfClient, function (index, item) {
            ...
            console.log(item);
            var ClientStatusHtml = "";
            if (item.LineStatusID === 3) {
                ClientStatusHtml = '<div style="color:green;font-weight:bold;">Active</div>';
            }
            if (item.LineStatusID === 4) {
                ClientStatusHtml = '<div style="color:red;font-weight:bold;">InActive</div>';
            }
            if (item.LineStatusID === 5) {
                ClientStatusHtml = '<div style="color:red;font-weight:bold;">Lock</div>';
            }

            $('#tblUsers tbody').append('<tr><td style="padding:0px"><input type="hidden" id="EmployeeDetailsID" name="EmployeeDetailsID" value=' + item.ClientDetailsID + '></td><td>' + item.Name + '</td><td>' + item.LoginName + '</td><td>' + item.PackageName + '</td><td>' + item.Address + '</td><td>' + item.Email + '</td><td>' + item.ZoneName + '</td><td>' + item.ContactNumber + '</td><td>' + ClientStatusHtml + '</td><td><a href="" id="ShowPopUps">Show</a></td></tr>');
            //$('#tblUsers tbody>tr:eq(' + index + ')').find("td:eq(0)").html("<input type='hidden' id='EmployeeDetailsID' name='EmployeeDetailsID' value=" + item.EmployeeDetailsID + ">");
            //$('#tblUsers tbody>tr:eq(' + index + ')').find("td:eq(1)").text(item.Name);
            //$('#tblUsers>tbody>tr:eq(' + index + ')').find("td:eq(2)").text(item.LoginName);
            //$('#tblUsers>tbody>tr:eq(' + index + ')').find("td:eq(3)").text(item.Package.PackageName);
    ...
    SearchActiveClientListByZoneError: function (data) {
